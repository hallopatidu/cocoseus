...
export function override(target:any, propertyKey: string, descriptor: PropertyDescriptor){
    if(DEV){
        if(!cocoseus_classify.hadInjectorImplemented(target.constructor as Constructor, ParasitifyName)){
...
function excuteHierarchyOverridding(thisComp:Component){ 
    ...
    const previousCompIndex:number = allNodeComponents.findIndex((component:Component, index:number, allComponents:Component[])=>{
        ...
        if(eligibleForInheritance){
            ...
            const componentIsParasite:boolean = cocoseus_classify.hadInjectorImplemented(component.constructor as Constructor, ParasitifyName);            
