...
interface IIntergerPicture { 
    type: string; 
    primary: string; 
    case: string; 
    ordinal: boolean;
    zeroCode?: number;
    mandatoryDigits?: number;
    optionalDigits?: number;
    groupingSeparators?: { position: number; character: string } | { position: number; character: string }[];
    regular?: boolean;
    token?: string;
}
...
const few: string[] = ['Zero', 'One', 'Two', 'Three', 'Four', 'Five', 'Six', 'Seven', 'Eight', 'Nine', 'Ten',
    'Eleven', 'Twelve', 'Thirteen', 'Fourteen', 'Fifteen', 'Sixteen', 'Seventeen', 'Eighteen', 'Nineteen'];
few.forEach(function (word: string, index: number): void {
    wordValues[word.toLowerCase()] = index;
});

const ordinals:string[] = ['Zeroth', 'First', 'Second', 'Third', 'Fourth', 'Fifth', 'Sixth', 'Seventh', 'Eighth', 'Ninth', 'Tenth',
    'Eleventh', 'Twelfth', 'Thirteenth', 'Fourteenth', 'Fifteenth', 'Sixteenth', 'Seventeenth', 'Eighteenth', 'Nineteenth'];
ordinals.forEach(function (word, index) {
    wordValues[word.toLowerCase()] = index;
});

const decades:string[] = ['Twenty', 'Thirty', 'Forty', 'Fifty', 'Sixty', 'Seventy', 'Eighty', 'Ninety', 'Hundred'];
...
const magnitudes:string[] = ['Thousand', 'Million', 'Billion', 'Trillion'];
...
const decimalGroups: number[] = [0x30, 0x0660, 0x06F0, 0x07C0, 0x0966, 0x09E6, 0x0A66, 0x0AE6, 0x0B66, 0x0BE6, 0x0C66, 0x0CE6, 0x0D66, 0x0DE6, 0x0E50, 0x0ED0, 0x0F20, 0x1040, 0x1090, 0x17E0, 0x1810, 0x1946, 0x19D0, 0x1A80, 0x1A90, 0x1B50, 0x1BB0, 0x1C40, 0x1C50, 0xA620, 0xA8D0, 0xA900, 0xA9D0, 0xA9F0, 0xAA50, 0xABF0, 0xFF10];

const defaultPresentationModifiers: Record<string, string> = {
...
const millisInADay:number = 1000 * 60 * 60 * 24;
// Regular expression to match an ISO 8601 formatted timestamp
const iso8601regex:RegExp = new RegExp('^\\d{4}(-[01]\\d)*(-[0-3]\\d)*(T[0-2]\\d:[0-5]\\d:[0-5]\\d)*(\\.\\d+)?([+-][0-2]\\d:?[0-5]\\d|Z)?$');
...
*/
function numberToWords(value: number, ordinal: boolean): string {
    const lookup = function (num: number, prev: boolean, ord: boolean): string {
        let words = '';
        if (num <= 19) {
            ...
            let mag = Math.floor(Math.log10(num) / 3);
 ...
 */
function decimalToLetters(value: number, aChar: string): string {
 ...
 * @returns {number} - the numeric value
 */
function lettersToDecimal(letters: string, aChar: string): number {
    const aCode = aChar.charCodeAt(0);
    let decimal = 0;
 ...
 */
export function formatInteger(value: number | undefined, picture: string): string | undefined {
 ...
 * @private
 */
function _formatInteger(value: number, format: any): string {
    ...
    switch (format.primary) {
        ...
        case formats.DECIMAL:
            ...
            const padLength = format.mandatoryDigits - formattedInteger.length;
            if (padLength > 0) {
                const padding = (new Array(padLength + 1)).join('0');
            ...
            if (format.regular) {
                ...
                format.groupingSeparators.reverse().forEach((separator: any) => {
            ...
            if (format.ordinal) {
                const suffix123: Record<string, string> = { '1': 'st', '2': 'nd', '3': 'rd' };
                const lastDigit = formattedInteger[formattedInteger.length - 1];
                let suffix = suffix123[lastDigit];
 ...
 */
function analyseIntegerPicture(picture: string): IIntergerPicture {
    const format: IIntergerPicture = {
        type: 'integer',
    ...
    let primaryFormat: string, formatModifier: string | undefined;
    ...
    switch (primaryFormat) {
        ...
        default: {
            ...
            let zeroCode: number | null = null;
            let mandatoryDigits = 0;
            let optionalDigits = 0;
            const groupingSeparators: { position: number; character: string }[] = [];
            ...
            formatCodepoints.forEach((codePoint) => {
                // step through each char in the picture to determine the digit group
                let digit = false;
                ...
                if (!digit) {
                    if (codePoint === 0x23) { // # - optional-digit-sign
                        ...
                        // neither a decimal-digit-sign nor optional-digit-sign, assume it is a grouping-separator-sign
            ...
            if (mandatoryDigits > 0) {
                ...
                const regularRepeat = function (separators: { position: number; character: string }[]) {
 ...
 */
function analyseDateTimePicture(picture: string): { type: string; parts: any[] } {
    const spec: any[] = [];
    const format = {
        type: 'datetime',
        parts: spec
    };
    const addLiteral = function (start: number, end: number): void {
        if (end > start) {
    ...
    while (pos < picture.length) {
        if (picture.charAt(pos) === '[') {
            ...
            const comma = marker.lastIndexOf(','); // 2. The width modifier may be recognized by the presence of a comma
            let presMod: string; // the presentation modifiers
            if (comma !== -1) {
                // §9.8.4.2 The Width Modifier
                const widthMod = marker.substring(comma + 1);
                const dash = widthMod.indexOf('-');
                let min, max;
                const parseWidth = function (wm: string | undefined): number | undefined {
            ...
            if (presMod.length === 1) {
                ...
                const lastChar = presMod.charAt(presMod.length - 1);
            ...
            if (def.presentation1[0] === 'n') {
                ...
                let integerPattern = def.presentation1;
                ...
                if (def.component === 'Y') {
                    ...
                    if (def.width && def.width.max !== undefined) {
                        ...
                        const w = def.integerFormat.mandatoryDigits + def.integerFormat.optionalDigits;
                        if (w >= 2) {
                            def.n = w;
                        }
                    }
...
function startOfFirstWeek(ym: { year: number; month: number }): number {
    ...
    let dayOfJan1 = (new Date(jan1)).getUTCDay();
    if (dayOfJan1 === 0) {
        dayOfJan1 = 7;
    }
...
function yearMonth(year: number, month: number): YearMonth {
    return {
        year: year,
        month: month,
        ...
        previousMonth: function (): YearMonth {
            return (month === 0) ? yearMonth(year - 1, 11) : yearMonth(year, month - 1);
        },
        nextYear: function (): YearMonth {
            return yearMonth(year + 1, month);
        },
        previousYear: function (): YearMonth {
            return yearMonth(year - 1, month);
        }
    };
};

function deltaWeeks(start: number, end: number): number {
 ...
 */
function formatDateTime(millis: number, picture?: string, timezone?: string): string {
    let offsetHours = 0;
    let offsetMinutes = 0;
    ...
    const formatComponent = (date: Date, markerSpec: any): string => {
        let componentValue = getDateTimeFragment(date, markerSpec.component);
 ...
 */
function generateRegex(formatSpec: any): any {
    const matcher: any = {};
    if (formatSpec.type === 'datetime') {
        matcher.type = 'datetime';
        matcher.parts = formatSpec.parts.map((part: any) => {
            const res: any = {};
            if (part.type === 'literal') {
                ...
                res.parse = function (value: string) {
                    if (part.component === 'z') {
                ...
                res.regex = generateRegex(part.integerFormat).regex;
                res.parse = generateRegex(part.integerFormat).parse;
            } else {
        ...
        switch (formatSpec.primary) {
            case formats.LETTERS:
                ...
                matcher.parse = function (value: string) {
            ...
            case formats.DECIMAL:
                ...
                matcher.parse = function (value: string) {
                    ...
                    if (formatSpec.regular) {
                        ...
                        formatSpec.groupingSeparators.forEach((sep: any) => {
 ...
 */
export function parseInteger(value: string | undefined, picture: string): number | undefined {
    if (typeof value === 'undefined') {
        return undefined;
    }
 ...
 */
function parseDateTime(timestamp: string | undefined, picture: string): number | undefined {
    ...
    if (info !== null) {
        ...
        let millis = Date.UTC(components.Y, components.M, components.D, components.H, components.m, components.s, components.f);
 ...
 */
export function toMillis(timestamp: string | undefined, picture?: string): number | undefined {
 ...
 * @returns {String} - the formatted timestamp
 */
export function fromMillis(millis: number | undefined, picture?: string, timezone?: string): string | undefined {
    // undefined inputs always return undefined
