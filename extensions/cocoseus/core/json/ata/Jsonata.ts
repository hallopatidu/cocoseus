...
import { formatInteger, fromMillis, parseInteger, toMillis } from "./DateTime";
import { parser } from "./Parser";

interface ResultWithTupleStream extends Array<any> {
    tupleStream?: boolean;
}

type JsonataProcedure = {
    ...
    body: any;
    thunk?: boolean;
    apply?: (self: any, args: any[]) => Promise<any>;
};
...
const isArrayOfStrings = Utils.isArrayOfStrings;
const isArrayOfNumbers = Utils.isArrayOfNumbers;
const createSequence = Utils.createSequence;
const isSequence = Utils.isSequence;
const isFunction = Utils.isFunction;
const isLambda = Utils.isLambda;
const isIterable = Utils.isIterable;
const isPromise = Utils.isPromise;
const getFunctionArity = Utils.getFunctionArity;
const isDeepEqual = Utils.isDeepEqual;
 ...
 */
async function evaluate(expr: any, input: any, environment: any): Promise<any> {
    let result;

    const entryCallback: ((expr: any, input: any, environment: any) => Promise<void>) | undefined = environment.lookup(Symbol.for('jsonata.__evaluate_entry'));
    if(entryCallback) {
 ...
 */
async function evaluatePath(expr: any, input: any, environment: any): Promise<any> {
    var inputSequence;
    ...
    if (expr.keepSingletonArray) {
        ...
        if (Array.isArray(resultSequence) && (resultSequence as any).cons && !(resultSequence as any).sequence) {
 ...
 * @returns 
 */
function createFrameFromTuple(environment: any, tuple: Record<string, any>): any {
 ...
 * @returns {*} Evaluated input data
 */
async function evaluateStep(expr: any, input: any[], environment: any, lastStep: boolean): Promise<any> {
    let result:any[];
    if (expr.type === 'sort') {
    ...
    if (lastStep && result.length === 1 && Array.isArray(result[0]) && !isSequence(result[0])) {
        ...
        result.forEach(function (res) {
            if (!Array.isArray(res) || (typeof res === 'object' && 'cons' in res)) {
 ...
 * @returns 
 */
async function evaluateStages(stages: any[], input: any, environment: any): Promise<any> {
    var result = input;
 ...
 * @returns {*} Evaluated input data
 */
async function evaluateTupleStep(
    expr: any,
    input: any[],
    tupleBindings: any[] | undefined,
    environment: any
): Promise<any[]> {
    // 
    let result: ResultWithTupleStream = [];
    if (expr.type === 'sort') {
        if (tupleBindings) {
            result = await evaluateSortExpression(expr, tupleBindings, environment);
            result.tupleStream = true; // Marking the result as a tuple stream
            const sorted = await evaluateSortExpression(expr, input, environment);
            result = createSequence();
            result.tupleStream = true;
            for (let ss = 0; ss < sorted.length; ss++) {
                const tuple: Record<string, any> = { '@': sorted[ss] };
                tuple[expr.index] = ss;
                result.push(tuple);
            }
        }
    ...
    result = createSequence();
    result.tupleStream = true;
    let stepEnv = environment;
    if (tupleBindings === undefined) {
        tupleBindings = input.map(item => ({ '@': item }));
    }

    for (let ee:number = 0; ee < tupleBindings.length; ee++) {
        stepEnv = createFrameFromTuple(environment as Record<string, any>, tupleBindings[ee] as Record<string, any>);
        let res: ResultWithTupleStream | undefined = await evaluate(expr, tupleBindings[ee]['@'], stepEnv);
        // res is the binding sequence for the output tuple stream
        if (typeof res !== 'undefined') {
            if (!Array.isArray(res)) {
                res = [res] as ResultWithTupleStream;
            }
            for (let bb = 0; bb < res.length; bb++) {
...
async function evaluateFilter(
    ...
    let results: ResultWithTupleStream = createSequence();
 ...
 * @returns {*} Evaluated input data
 */
async function evaluateBinary(expr: any, input: any, environment: any): Promise<any> {
    let result;
    const lhs:any = await evaluate(expr.lhs, input, environment);
    var op = expr.value;

    //defer evaluation of RHS to allow short-circuiting
    const evalrhs = async () => await evaluate(expr.rhs, input, environment);
    if (op === "and" || op === "or") {
        try {
 ...
 * @returns {*} Evaluated input data
 */
async function evaluateUnary(expr: any, input: any, environment: any): Promise<any> {
    var result;
...
function evaluateName(expr, input, environment) {
    ...
    return Functions.lookup(input, expr.value);
}
 ...
 * @returns {*} Evaluated input data
 */
function evaluateLiteral(expr: any, input?: any, environment?: any) {
    return expr.value;
}
 ...
 * @returns {*} Evaluated input data
 */
function evaluateWildcard(expr: any, input: any, environment?: any): any {
    var results = createSequence();
    if (Array.isArray(input) && (input as any & { outerWrapper?: boolean }).outerWrapper && input.length > 0) {
        input = input[0];
    }
 ...
 */
function flatten(arg: any, flattened: any[] = []): any[] {
 ...
 * @returns {*} Evaluated input data
 */
function evaluateDescendants(expr: any, input: any, environment?:any): any {
    var result;
    var resultSequence = createSequence();
 ...
 */
function recurseDescendants(input: any, results: any[]): void {
 ...
 * @returns {*} Result
 */
function evaluateNumericExpression(lhs: any, rhs: any, op: string): number | undefined {
 ...
 * @returns {*} Result
 */
function evaluateEqualityExpression(lhs: any, rhs: any, op: string): boolean {
    let result: boolean;

    // type checks
    const ltype: string = typeof lhs;
    const rtype: string = typeof rhs;
 ...
 * @returns {*} Result
 */
function evaluateComparisonExpression(lhs: any, rhs: any, op: string): boolean | undefined {
    ...
    const lcomparable: boolean = (ltype === 'undefined' || ltype === 'string' || ltype === 'number');
    const rcomparable: boolean = (rtype === 'undefined' || rtype === 'string' || rtype === 'number');

    // if either lhs or rhs are not comparable (string or numeric) values, then throw an error
    ...
    // if lhs and rhs are not of the same type
    if (ltype !== rtype) {
        throw {
            code: "T2009",
            stack: (new Error()).stack,
 ...
 */
function evaluateIncludesExpression(lhs: any, rhs: any): boolean {
    let result = false;
 ...
 * @returns {*} Result
 */
async function evaluateBooleanExpression(lhs: any, evalrhs: () => Promise<any>, op: string): Promise<boolean> {
    let result: boolean;

    const lBool: boolean = boolize(lhs);
...
function boolize(value) {
    const booledValue = Functions.boolean(value);
 ...
 */
function evaluateStringConcat(lhs: any, rhs: any): string {
    let result: string;
    let lstr: string = '';
    let rstr: string = '';
    if (typeof lhs !== 'undefined') {
        lstr = Functions.string(lhs);
    }
    if (typeof rhs !== 'undefined') {
        rstr = Functions.string(rhs);
    }
 ...
 * @returns {{}} Evaluated input data
 */
async function evaluateGroupExpression(
    expr: any,
    input: any,
    environment: any
): Promise<Record<string, any>> {
    var result: Record<string, any> = {};
    var groups: Record<string, { data: any; exprIndex: number }> = {};
    ...
    let generators = await Promise.all(
        Object.keys(groups).map(async (key, idx) => {
 ...
 * @returns 
 */
function reduceTupleStream(tupleStream: any[]): Record<string, any> {
    if (!Array.isArray(tupleStream)) {
        return tupleStream;
    }
    const result: Record<string, any> = {};
    Object.assign(result, tupleStream[0]);
    for (let ii = 1; ii < tupleStream.length; ii++) {
        for (const prop in tupleStream[ii]) {
            result[prop] = Functions.append(result[prop], tupleStream[ii][prop]);
        }
    }
    return result;
}
 ...
 */
function evaluateRangeExpression(lhs: number | undefined, rhs: number | undefined): number[] | undefined {
    let result: number[] | undefined;
    ...
    for (let item = lhs, index = 0; item <= rhs; item++, index++) {
        result[index] = item;
    }
    (result as any).sequence = true;
    return result;
}
 ...
 * @returns {*} Evaluated input data
 */
async function evaluateBindExpression(expr: any, input: any, environment: any): Promise<any> {
    // The RHS is the expression to evaluate
    // The LHS is the name of the variable to bind to - should be a VARIABLE token (enforced by parser)
    const value = await evaluate(expr.rhs, input, environment);
 ...
 * @returns {Promise<any>} Evaluated input data
 */
async function evaluateCondition(expr: any, input: any, environment: any): Promise<any> {
    let result: any;
    const condition = await evaluate(expr.condition, input, environment);
    if (Functions.boolean(condition)) {
 ...
 * @returns {*} Evaluated input data
 */
async function evaluateBlock(expr: any, input: any, environment: any): Promise<any> {
    let result: any;
 ...
 */
function evaluateRegex(expr: { value: string; position?: number }, input?:any, environment?:any) {
    var re = new (jsonata as any).RegexEngine(expr.value);
    var closure = function (str: string, fromIndex?: number) {
        var result: {
            match: string;
            start: number;
            end: number;
    ...
    async function evaluateSortExpression(expr, input, environment) {
        ...
        result = await Functions.sort.apply(focus, [lhs, comparator]);
    ...
    const chainAST: any = parser('function($f, $g) { function($x){ $g($f($x)) } }');
    ...
    async function evaluateApplyExpression(
        ...
        let result: any;
        const lhs = await evaluate(expr.lhs, input, environment);
        if (expr.rhs.type === 'function') {
            // this is a function _invocation_; invoke it with lhs expression as the first argument
            result = await evaluateFunction(expr.rhs, input, environment, { context: lhs });
        } else {
            const func = await evaluate(expr.rhs, input, environment);
            ...
            if (isFunction(lhs)) {
                ...
                // λ($f, $g) { λ($x){ $g($f($x)) } }
                const chain = await evaluate(chainAST, null, environment);
    ...
    async function evaluateFunction(
        ...
        for (var jj = 0; jj < expr.arguments.length; jj++) {
            ...
            if (isFunction(arg)) {
                ...
                const closure = async function (...params: any[]) {
                    // invoke func
    ...
    function getNativeFunctionArguments(func: Function): string[] {
        ...
        const sigParens = /\(([^)]*)\)/.exec(signature)?.[1] || ''; // the contents of the parens
        const sigArgs = sigParens.split(',').map(arg => arg.trim()).filter(arg => arg !== '');
        return sigArgs;
    }
     ...
     */
    function defineFunction(func: Function, signature?: string) {
        const definition: { _jsonata_function: boolean; implementation: Function; signature?: any } = {
            _jsonata_function: true,
            implementation: func
        };
        if (typeof signature !== 'undefined') {
            definition.signature = Signature.parseSignature(signature);
        }
        return definition;
    }
     ...
     */
    async function functionEval(expr: string, focus?: any): Promise<any> {
        // undefined inputs always return undefined
        ...
        try {
            const ast = parser(expr, false);
            try {
                const result = await evaluate(ast, input, this.environment);
                return result;
            } catch (err) {
     ...
     */
    function functionClone(arg: any): any {
        // undefined inputs always return undefined
        if (typeof arg === 'undefined') {
            return undefined;
        }

        return JSON.parse(Functions.string(arg));
    }
     ...
     */
    function createFrame(enclosingEnvironment: any): {
        bind: (name: string, value: any) => void;
        lookup: (name: string) => any;
        timestamp: Date | null;
        async: boolean;
        isParallelCall: boolean;
        global: { ancestry: (null | any)[] };
    } {
        var bindings: Record<string, any> = {};
        const newFrame = {
            bind: function (name: string, value: any) {
                bindings[name] = value;
            },
    ...
    staticFrame.bind('length', defineFunction(Functions.getLength, '<s-:n>'));
    ...
    staticFrame.bind('boolean', defineFunction(Functions.boolean, '<x-:b>'));
    ...
    staticFrame.bind('assert', defineFunction(Functions.assert, '<bs?:x>'));
    ...
    staticFrame.bind('toMillis', defineFunction(toMillis, '<s-s?:n>'));
    staticFrame.bind('fromMillis', defineFunction(fromMillis, '<n-s?s?:s>'));
 ...
 */
function populateMessage(err: { code: string; message?: string; [key: string]: any }) {
    var template = errorCodes[err.code];
    if (typeof template !== 'undefined') {
        // if there are any handlebars, replace them with the field references
        // triple braces - replace with value
        // double braces - replace with json stringified value
        var message = template.replace(/\{\{\{([^}]+)}}}/g, function (_match, field) {
            return err[field];
        });
        message = message.replace(/\{\{([^}]+)}}/g, function (_match, field) {
            return JSON.stringify(err[field]);
        });
        err.message = message;
    }
 ...
 */
function jsonata(expr: string, options?: { recover?: boolean; RegexEngine?: any }): {
    evaluate: (input: any, bindings?: Record<string, any>, callback?: (err: any, result: any) => void) => Promise<any>;
    assign: (name: string, value: any) => void;
    registerFunction: (name: string, implementation: Function, signature?: string) => void;
    ast: () => any;
    errors: () => any;
} {
    (jsonata as any).RegexEngine = RegExp; // Default RegexEngine
    var ast: any;
    var errors: any;
    try {
    ...
    environment.bind('now', defineFunction(function (picture?: string, timezone?: string) {
        return fromMillis(timestamp.getTime(), picture, timezone);
    }, '<s?s?:s>'));
    ...
    if (options && options.RegexEngine) {
        // Define RegexEngine as a property of jsonata
        (jsonata as any).RegexEngine = options.RegexEngine;
    } else {
        (jsonata as any).RegexEngine = RegExp;
    }

    return {
        evaluate: async function (input: any, bindings?: Record<string, any>, callback?: (err: any, result: any) => void) {
        ...
        assign: function (name: string, value: any) {
            environment.bind(name, value);
        },
        registerFunction: function (name: string, implementation: Function, signature?: string) {
...
export const Jsonata: Function = jsonata
