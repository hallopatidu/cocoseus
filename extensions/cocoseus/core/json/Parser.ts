...
const escapes:Record<string,string> = {  // JSON string escape sequences - see json.org
    '"': '"',
    '\\': '\\',
    '/': '/',
    'b': '\b',
...
function tokenizer(path: string): (prefix?: boolean) => { type: string; value: any; position: number } | null {
    let position = 0;
    const length = path.length;

    const create = (type: string, value: any) => {
        return { type, value, position };
    };

    const scanRegex = (): RegExp => {
        let start = position;
        let depth = 0;
        let pattern: string;
        let flags: string;

        const isClosingSlash = (position: number): boolean => {
    ...
    const next = (prefix?: boolean): { type: string; value: any; position: number } | null => {
            ...
            while (position < length) {
                ...
                if (currentChar === '\\') {
                    ...
                    if (Object.prototype.hasOwnProperty.call(escapes, currentChar)) {
                        ...
                        const octets = path.substr(position + 1, 4);
                        if (/^[0-9a-fA-F]+$/.test(octets)) {
                            const codepoint = parseInt(octets, 16);
        ...
        const match = numregex.exec(path.substring(position));
        if (match !== null) {
        ...
        if (currentChar === '`') {
            ...
            if (end !== -1) {
                const name = path.substring(position, end);
                position = end + 1;
        ...
        for (; ;) {
            ...
            if (i === length || ' \t\n\r\v'.indexOf(ch) > -1 || Object.prototype.hasOwnProperty.call(operators, ch)) {
                if (path.charAt(position) === '$') {
                    const name = path.substring(position + 1, i);
                    position = i;
                    return create('variable', name);
                } else {
                    const name = path.substring(position, i);
                    position = i;
                    switch (name) {
                        case 'or':
                        case 'in':
...
export function parser(source: string, recover: boolean): any {
    let node;
    let lexer:Function;
    let symbol_table = {};
    let errors = [];
    const base_symbol = {
        nud: function () {
            ...
            var err: { code: string; token: string; position: number; remaining?: any[]; type?: string; stack?: string } = {
                code: 'S0211',
                token: this.value,
    ...
    function symbol(id: string, bp?: number): any {
    ...
    function handleError(err: { code: string; token?: string; position: number; remaining?: any[]; type?: string; stack?: string,value?:number }): any {
        if (recover) {
            // tokenize the rest of the buffer and add it to an error token
            err.remaining = remainingTokens();
            errors.push(err);
            const symbol = symbol_table["(error)"];
            const errorNode = Object.create(symbol);
            errorNode.error = err;
            errorNode.type = "(error)";
            node = errorNode;
            return node;
        } else {
            (err as { code: string; position: any; token: any; stack?: string }).stack = (new Error()).stack;
            throw err;
        }
    };


    function advance(id?: string, infix?: boolean): any {
        if (id && node.id !== id) {
            let code: string;
            if (node.id === '(end)') {
            ...
            const err = {
                ...
                token: node.value,
                value: id ? id.length : 0 // Convert `id` to a number (e.g., its length)
            };
            return handleError(err);
        }
        const next_token = lexer(infix);
        if (next_token === null) {
    ...
    function expression(rbp: number): any {
        let left: any;
        let t = node;
        advance(null, true);
        left = t.nud();
    ...
    // left associative
    function infix(id: string, bp?: number, led?: (left: any) => any): any {
        const bindingPower = bp || operators[id];
        const s = symbol(id, bindingPower);
        s.led = led || function (left: any): any {
            this.lhs = left;
    ...
    // <expression> <operator> <expression>
    // right associative
    function infixr(id: string, bp: number, led?: (left: any) => any): any {
    ...
    // <operator> <expression>
    function prefix(id: string, nud?: () => any): any {
        const s = symbol(id);
        s.nud = nud || function (): typeof s {
    ...
    infix("(", operators['('], function (left) {
        ...
        if (left.type === 'name' && (left.value === 'function' || left.value === '\u03BB')) {
            ...
            this.arguments.forEach(function (arg: any, index: number) {
            ...
            if (node.id === '<') {
                ...
                advance('>');
                try {
                    this.signature = Signature.parseSignature(sig);
                } catch (err: any) {
    ...
    prefix("[", function () {
        ...
        if (node.id !== "]") {
            for (; ;) {
                ...
                if (node.id === "..") {
                    // range operator
                    var range = {type: "binary", value: "..", position: node.position, lhs: item, rhs: null};
                    advance("..");
    ...
    function objectParser(left?: any): any {
        var a: [any, any][] = [];
        if (node.id !== "}") {
            for (; ;) {
                var n = expression(0);
    ...
    function tailCallOptimize (expr) {
        var result;
        if (expr.type === 'function' && !expr.predicate) {
            var thunk = {type: 'lambda', thunk: true, arguments: [], position: expr.position, body: null as any};
            thunk.body = expr;
            result = thunk;
    ...
    function pushAncestry(result, value) {
        ...
        function processAST (expr) {
            ...
            switch (expr.type) {
                case 'binary':
                    switch (expr.value) {
                        ...
                        case '^':
                            ...
                            var sortStep = {type: 'sort', position: expr.position, terms: []};
                            sortStep.terms = expr.rhs.map(function (terms) {
                                const expression = processAST(terms.expression);
                ...
                default:
                    ...
                    var err: { code: string; position: number; token: string; stack?: string } = {
                        code: code,
                        position: expr.position,
                        token: expr.value
                    };
